...
/*----------------------------------------------------------------------
Register Allocation : Register is allocated starting from R0 to R7.
...
int func_typecheck(char *name,struct ArgStruct *fargTable,int rettype,int type){
	...
	if(p==NULL){
		...
		while(arglist!=NULL && q!=NULL){
			printf("%s{%d}--%s{%d}",arglist->name,arglist->type,q->name,q->type);
			if(!(strcmp(arglist->name,q->name)) && arglist->ptrType==q->ptrType && arglist->type==q->type){
...
int getArgCount() {
	struct ArgStruct * dummy = fargTable;
	int count = 0;
	while(dummy != NULL) {
		count++;
		dummy = dummy->next;
